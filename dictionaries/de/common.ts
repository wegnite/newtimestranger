export const common = {
  buttons: {
    submit: "Absenden",
    cancel: "Abbrechen",
    confirm: "Bestätigen",
    back: "Zurück",
    next: "Weiter",
    save: "Speichern",
    delete: "Löschen",
    edit: "Bearbeiten",
    view: "Ansehen",
    download: "Herunterladen",
    close: "Schließen",
  },
  messages: {
    loading: "Lädt...",
    success: "Operation erfolgreich",
    error: "Ein Fehler ist aufgetreten",
    confirm: "Sind Sie sicher?",
    required: "Dieses Feld ist erforderlich",
    invalidInput: "Ungültige Eingabe",
  },
  navigation: {
    home: "Startseite",
    about: "Über",
    contact: "Kontakt",
    features: "Funktionen",
    pricing: "Preise",
    guides: "Leitfäden",
    settings: "Einstellungen",
  },
  activePlayers: "Aktive Spieler",
  countries: "Länder",
  coffeeRecipes: "Stufen",
  userRating: "Benutzerbewertung",
  localeSuggest: {
    switchToTitle: "Auf {langName} umschalten?",
    currentLangDesc: "Aktuelle Sprache ist {langName}.",
    dismissPermanent: "Nicht erneut anzeigen",
    switchToAlt: "Auf {langName} umschalten",
    switchToButton: "Umschalten",
  },
  onlineGames: {
    bannerTitle: "Möchten Sie direkt Spiele spielen?",
    bannerDescription:
      "Erleben Sie Digimon Story Time Stranger und weitere Spiele direkt in Ihrem Browser, keine Herunterladung erforderlich",
    playNowButton: "Jetzt Spielen",
    featured: "Empfohlene Spiele",
    viewAllGames: "Alle Spiele ansehen",
    levelCompletionText: "Spielen Sie Digimon Story Time Stranger Online!",
  },
} as const;
