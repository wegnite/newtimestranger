export const faq = {
  title: "Knit OutのFAQ",
  subtitle: "Knit Outに関するよくある質問",
  questions: [
    {
      question: "Knit Outをどのように遊ぶのですか？",
      answer:
        "あなたの課題は、下のグリッドから適切なボビンを選んでロープを集めることで、上から色鮮やかな結び目の列をクリアすることです。",
    },
    {
      question: "Knit Outは無料ですか？",
      answer:
        "このゲームは無料で遊べますが、アプリ内課金があり、広告も含まれています。",
    },
    {
      question: "レベルで引き戻された場合どうすればいいですか？",
      answer:
        "一部のレベルは難しい場合があります。ゲームではコインで獲得できるパワーアップが用意されています。進路が不明な場合、いくつかのプレイヤーは熟考した推測をすることをおすすめしています。",
    },
    {
      question: "オフラインで遊べますか？",
      answer: "はい、ユーザーのレビューによると、Knit Outはオフラインで遊べます。",
    },
    {
      question: "既知の問題はありますか？",
      answer:
        "一部のユーザーが画面の点滅などのバグを報告しています。問題が発生した場合は、ゲームを再起動するか、サポートに連絡することができます。",
    },
    {
      question: "サポートに連絡するにはどうすればいいですか？",
      answer:
        "info@superblast.gamesまでメールでサポートに連絡するか、App StoreまたはGoogle Playのアプリサポートリンクを通じて連絡することができます。",
    },
  ],
} as const;
