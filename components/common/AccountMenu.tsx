...
const AccountMenu = ({ user }: any) => {
	...
	const handleCancelSubscription = async () => {
		try {
			const response = await fetch('/api/stripe/payment/cancel', {
				method: 'POST',
			});
	...
	return (
		<div className="bg-background p-2">
			<div className='mb-2 flex items-center border-b border-border px-4 pb-3.5 pt-2'>
				<div className='mr-3'>
					<img
						src={profilePic}
				...
				<div>
					<p className='font-satoshi text-base font-medium text-foreground'>
						{user?.name}
					</p>
				...
				<ul className=''>
					{!isSubscribed ? (
						<li>
							...
							>
								<span className='flex w-full items-center gap-2 rounded-lg px-3.5 py-2 font-satoshi font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-accent-foreground'>
									<span>
										<svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
											<path d="M9 1.5V16.5M16.5 9H1.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
										</svg>
									</span>
									Subscribe Ad-Free
								</span>
							...
							<li>
								...
								>
									<span className='flex w-full items-center gap-2 rounded-lg px-3.5 py-2 font-satoshi font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-accent-foreground'>
										<span>
											<svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
												<path d="M14.25 3.75L3.75 14.25M3.75 3.75L14.25 14.25" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
											</svg>
										</span>
										Cancel Subscription
									</span>
					...
					<li className='mt-2.5 border-t border-border pt-2.5'>
